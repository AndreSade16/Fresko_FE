...
function RecipeEditModal({
  ...
  const [cost, setCost] = useState(selectedItem?.cost ?? "CHEAP");
  const [procedure, setProcedure] = useState(selectedItem?.procedure ?? "");
  ...
  >(
    selectedItem?.ingredients.map((recipeIngredient) => {
      return {
        ingredientDefinitionId:
          recipeIngredient.ingredientDefinition.ingredientDefinitionId,
        name: recipeIngredient.ingredientDefinition.name,
        unit: recipeIngredient.ingredientDefinition.unit,
        quantityPerPerson: recipeIngredient.quantityPerPerson,
      };
    }) ?? [],
  );
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    try {
      ...
      const recipeRes = await apiFetch<RecipePageContent>(
        `/recipes/${selectedItem?.recipeId}`,
        {
          method: "PUT",
          body: formData,
        },
